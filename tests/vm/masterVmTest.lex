...
let files, listErr = fs.listDir("tests/vm")
if listErr != null {
    println("ERROR: cannot list tests/vm: " + listErr)
    return null
}

// Collect .lex files, excluding ourselves.
let tests = makeArray(len(files), "")
let testCount = 0
let i = 0
while i < len(files) {
    let name = files[i]
    let parts = split(name, ".")
    if parts[len(parts) - 1] == "lex" && name != "masterVmTest.lex" {
        tests[testCount] = name
...
println("")
println("  kLex VM Master Test Suite")
println("  ===========================")
println("")

let passed = makeArray(total, "")
let passedCount = 0
let failed = makeArray(total, "")
let failedCount = 0
...
while i < total {
    ...
    // Run the sub-script through the bytecode VM, capturing all output so
    // nothing leaks to the terminal.
    let stdout, stderr, exitCode, runErr = _processExec(interpreter, ["--vm", "tests/vm/" + name])

    // Determine pass/fail.
    // exitCode != 0 → parse or VM error (main.go calls os.Exit(1) on failures).
...
println("")
println("  ===========================")
println("  Passed : " + str(passedCount) + " / " + str(total))
println("  Failed : " + str(failedCount) + " / " + str(total))
println("")
