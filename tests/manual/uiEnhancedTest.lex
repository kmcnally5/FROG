...
let dropItems = ["Option Alpha", "Option Beta", "Option Gamma", "Option Delta", "Option Epsilon"]
let listItems = ["Apple", "Banana", "Cherry", "Date", "Elderberry",
             "Fig", "Grape", "Honeydew", "Kiwi", "Lemon",
             "Mango", "Nectarine", "Orange", "Papaya", "Quince"]
let tabLabels = ["Controls", "Layout", "Data", "Visual"]
...
window(980, 720, "kLex UI — Enhanced", fn(frame) {
    ...
    let pad  = 10.0
    let tabH = 34.0
    let tabY = pad + tabH + 4.0       // content starts below tab bar
    let ch   = wh - tabY - pad        // content height
    let colW = (ww - pad * 4.0) / 3.0 // three equal columns
    ...
    if activeTab == 0 {
        ...
        let sy = ui.layoutY()
        let sw = (colW - 24.0) / 6.0
        for ci in range(0, 6) {
            let colors = [[1.0, 0.3, 0.3], [0.3, 1.0, 0.4], [0.3, 0.5, 1.0],
    ...
    if activeTab == 2 {
        let listW   = ww * 0.38
        let detailX = pad * 2.0 + listW
        let detailW = ww - detailX - pad
        ...
        let detailY2 = tabY + ch * 0.42 + 8.0
        let detailH2 = ch - ch * 0.42 - 8.0
