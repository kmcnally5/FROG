...
if err == null {
    ...
    let received = makeArray(0)
    let notifTask = async(fn() {
        let ch = notifCh
        let buf = makeArray(0)
        while true {
            let msg, ok = recv(ch)
            if !ok { return buf }
            let newBuf = makeArray(len(buf) + 1)
            let i = 0
    ...
    let result, cerr = bridgeCall(bridge, "slow_with_notifications", [10])
    bridgeClose(bridge)
...
if err == null {
    ...
    let _b = bridge
    let t1 = async(fn() { return bridgeCall(_b, "echo_concurrent", ["A", 80]) })
    let t2 = async(fn() { return bridgeCall(_b, "echo_concurrent", ["B", 20]) })
    let t3 = async(fn() { return bridgeCall(_b, "echo_concurrent", ["C", 60]) })
    let t4 = async(fn() { return bridgeCall(_b, "echo_concurrent", ["D", 40]) })
...
let rb, err = br.newResilient("python3", [BRIDGE_PATH], {"require_deps": false})
expect("newResilient succeeds", err == null)
if err == null {
    let result, cerr = rb.call("echo", ["hello from resilient"])
...
if err == null {
    ...
    let result, cerr = rb.call("kill_self", [])
    ...
    let result2, cerr2 = rb.call("echo", ["after restart"])
...
if err == null {
    ...
    let _, cerr = rb.call("echo", ["should not reach bridge"])
    ...
    rb.reset()
    let result, cerr2 = rb.call("echo", ["after reset"])
...
let result, err = br.withBridge("python3", [BRIDGE_PATH], null, fn(b) {
