#!/bin/bash
# serve.sh — serve the Canvas2D graphics WASM test.
#
# Usage: from tests/wasm_graphics/, run ./serve.sh
...
# Ensure bin/klex.wasm is the one serve.lex will read, in case KLEX_WASM was overridden.
...
echo "==> starting kLex Canvas2D test — http://localhost:8766/"
