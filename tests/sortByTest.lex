...
println(str(arr.sort([3, 1, 4, 1, 5, 9, 2, 6])))

// 2. sortBy ascending integers
println(str(arr.sortBy([5, 2, 8, 1, 9], fn(a, b) { return a < b })))

// 3. sortBy descending integers
println(str(arr.sortBy([5, 2, 8, 1, 9], fn(a, b) { return a > b })))

// 4. sortBy strings ascending
println(str(arr.sortBy(["banana", "apple", "cherry"], fn(a, b) { return a < b })))

// 5. sortBy strings by length
println(str(arr.sortBy(["banana", "fig", "apple", "kiwi"], fn(a, b) { return len(a) < len(b) })))
...
byAge = arr.sortBy(people, fn(a, b) { return a.age < b.age })
for p in byAge {
    println(p.name + " " + str(p.age))
}

// 7. sortBy structs descending by field
byAgeDesc = arr.sortBy(people, fn(a, b) { return a.age > b.age })
for p in byAgeDesc {
    println(p.name + " " + str(p.age))
}
...
nums = [3, 1, 2]
sorted = arr.sortBy(nums, fn(a, b) { return a < b })
