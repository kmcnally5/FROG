...
// --- parse object ---
let data, err = json.parse(`{"name":"Frog","age":42}`)
...
// --- parse array ---
let arr, err = json.parse("[1,2,3]")
println(err == null)          // true
println(arr[0])               // 1
println(arr[2])               // 3

// --- parse nested ---
let nested, err = json.parse(`{"user":{"name":"Alice","scores":[10,20,30]}}`)
println(err == null)                         // true
println(nested["user"]["name"])              // Alice
println(nested["user"]["scores"][1])         // 20

// --- parse booleans and null ---
let bools, err = json.parse("[true,false,null]")
println(err == null)          // true
...
// --- parse float ---
let fdata, err = json.parse("3.14")
println(err == null)          // true
println(fdata)                // 3.14

// --- parse error ---
let bad, err = json.parse(`{bad}`)
println(err != null)          // true
