import "stdlib/merkle.lex" as merkle
import "stdlib/hash.lex" as h

// Test 1: Basic Merkle tree root computation
let content = "block_data_0_replica_content_12345"
let root = merkle.tree_root(content, merkle.CHUNK_SIZE)
println("[test] content length: " + str(len(content)))
println("[test] root hash: " + str(root))

// Test 2: Get proof for chunk 0
let proof_result = merkle.get_proof(content, 0, merkle.CHUNK_SIZE)
let chunk_hash = proof_result[0]
let path = proof_result[1]
