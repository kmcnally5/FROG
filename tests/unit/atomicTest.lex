...
let af = atomicFloatArray(10, 1.5)
...
let ai2 = atomicIntArray(5, 100)

// Successful CAS: current value matches expected
let swapped = atomicCAS(ai2, 0, 100, 999)
...
let swapped2 = atomicCAS(ai2, 0, 100, 555)
...
let final = atomicLoad(counter, 0)
...
let fcounter = atomicFloatArray(1)

let ftasks = makeArray(10, null)
for w in range(0, 10) {
  let fc = fcounter
  ftasks[w] = async(fn() {
    for i in range(0, 10000) {
...
let ffinal = atomicLoad(fcounter, 0)
// 10 workers × 10000 × 0.1 = 10000.0 (with floating-point fuzz)
let diff = ffinal - 10000.0
...
let nums = makeArray(1000, 0)
for i in range(0, 1000) {
  nums[i] = i + 1
}

let total = atomicIntArray(1)
parallelArrayForEach(nums, fn(v, i) {
  atomicAdd(total, 0, v)
})

let result = atomicLoad(total, 0)
...
let big = atomicFloatArray(10000)
let items = makeArray(1000000, 0)
