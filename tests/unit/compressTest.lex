import "stdlib/compress.lex" as c
...
let compressed, err = c.compress(data)
...
let decompressed, err = c.decompress(compressed)
...
let deflated, err = c.deflate(data)
...
let inflated, err = c.inflate(deflated)
...
let testData = "test data test data test data"
let size = c.compressedSize(testData)
...
let items = ["item1 " + "data", "item2 " + "data", "item3 " + "data"]
let compressed_items, err = c.compressMany(items)
...
let decompressed_items, err = c.decompressMany(compressed_items)
...
// Create a test file
let testFile = "test_compress_file.txt"
let testFileCompressed = "test_compress_file.txt.gz"
let content = "This is test content for compression. " + "This is test content for compression. " + "This is test content for compression."
...
if err == null {
    ...
    if err == null {
        ...
        let compressedData, err = fs.read(testFileCompressed)
