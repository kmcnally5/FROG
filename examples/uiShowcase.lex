...
let activeTab  = 0
let tabLabels  = ["Inputs", "Selection", "Display", "Typography", "Panels"]
...
let dropItems = ["Alpha",  "Beta", "Gamma", "Delta", "Epsilon",
             "Zeta",   "Eta",  "Theta", "Iota",  "Kappa"]

let listItems = ["Amphibian", "Axolotl",   "Bullfrog",  "Caecilian",
...
window(1100, 760, "kLex UI Showcase — All Widgets", fn(frame) {
    ...
    let tabY = pad + 40.0
    let ch   = wh - tabY - pad
    let colW = (ww - pad * 4.0) / 3.0

    // ── Tab 0: Inputs ─────────────────────────────────────────────────────────
    if activeTab == 0 {
        let col1X = pad
        let col2X = pad * 2.0 + colW
        let col3X = pad * 3.0 + colW * 2.0
    ...
    if activeTab == 1 {
        let leftW   = ww * 0.40
        let rightX  = pad * 2.0 + leftW
        let rightW  = ww - rightX - pad
        let halfH   = ch * 0.5 - 4.0
        ...
        let detailY = tabY + halfH + 8.0
        let detailH = ch - halfH - 8.0
    ...
    if activeTab == 2 {
        ...
        // Left-bottom — spinner
        let spinY = tabY + ch * 0.55 + 8.0
        let spinH = ch - ch * 0.55 - 8.0
        ...
        let swatchY = tabY + ch * 0.55 + 8.0
        let swatchH = ch - ch * 0.55 - 8.0
    ...
    // ── Tab 3: Typography ─────────────────────────────────────────────────────
    if activeTab == 3 {
        let halfW  = (ww - pad * 3.0) * 0.5
        let rightX = pad * 2.0 + halfW
    ...
    if activeTab == 4 {
        ...
        // panelTitle() below
        let pBY = tabY + halfH + 8.0
        let pBH = ch - halfH - 8.0
        ...
        let shBY = tabY + halfH + 8.0
        let shBH = ch - halfH - 8.0
        ...
        ui.layoutSeparator()
        let fileItems = ["readme.md", "main.lex", "stdlib/", "tests/", "go.mod", "go.sum"]
        ...
        let fpFY = tabY + halfH + 8.0
        let fpFH = ch - halfH - 8.0
        ...
        ui.layoutSeparator()
        let srcItems = ["eval.go", "object.go", "env.go", "typecheck.go", "builtins_ui.go"]
