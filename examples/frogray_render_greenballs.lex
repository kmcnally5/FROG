...
// ============================================================================
// 
// It was supposed to be a cute frog modeled with geometric primitives and 
// rendered with full ray tracing: shadows, reflections, and Phong shading.
// Sadly, it is not...  But give it a shot anyway cause it makes a nice random
// AI generated artwork :)

import "stdlib/frogray.lex" as frogray

// Create materials
let mat_head = frogray.create_material(0.2, 0.7, 0.3, 0.1, 64)      // Green head
let mat_body = frogray.create_material(0.15, 0.65, 0.25, 0.15, 64)  // Darker green body
let mat_eye = frogray.create_material(0.9, 0.9, 0.95, 0.3, 128)      // Shiny white eyes
let mat_pupil = frogray.create_material(0.1, 0.1, 0.1, 0.2, 64)      // Black pupils
let mat_belly = frogray.create_material(0.5, 0.85, 0.4, 0.05, 32)    // Lighter belly
let mat_leg = frogray.create_material(0.18, 0.62, 0.28, 0.1, 48)     // Leg green
let mat_ground = frogray.create_material(0.4, 0.5, 0.3, 0.0, 16)     // Ground (no reflection)
let mat_sky = frogray.create_material(0.5, 0.7, 1.0, 0.2, 8)         // Sky (mild reflection)
...
// Render the scene
let pixels = frogray.render_parallel(width, height, camera, objects, lights, num_workers)

println("✓ Rendering complete!")
println("")

// Write to file
let output_file = "frog_scene.ppm"
