...
// Start YARA bridge
let yaraB, err = sh.startYaraBridge("examples/SecretHunter/secrets.yar")
if err != null {
...
let files = ["/tmp/sh_yara_test.txt"]
let matches, err = bridgeCall(yaraB, "scan_batch", [files])
if err != null { println("scan error: {err.message}")  return }
let findings = makeArray(len(matches))
let i = 0
while i < len(matches) {
    let hit = matches[i]
